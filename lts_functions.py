...
def is_bike_lane(gdf_edges):
    ...
    if "shoulder:access:bicycle" in gdf_edges.columns:
        lane_check = (
            np.any(gdf_edges[cycleway_tags].isin(lane_identifiers), axis=1)
...
def get_max_speed(
    gdf_edges, national=40, local=50, motorway=100, primary=80, secondary=80
):
    """
    ...
    gdf_edges["maxspeed_assumed"] = np.select(
        conditions, values, default=gdf_edges["maxspeed"]
    )

    # if multiple speed values present, use the largest one
    gdf_edges["maxspeed_assumed"] = (
        gdf_edges["maxspeed_assumed"]
        .apply(lambda x: np.array(x, dtype="int"))
        .apply(lambda x: np.max(x))
    )
...
def bike_lane_analysis_with_parking(gdf_edges):
    ...
    gdf_edges["width"] = gdf_edges["width"].astype(float)
...
def bike_lane_analysis_no_parking(gdf_edges):
    ...
    gdf_edges.loc[
        gdf_edges[["width"]].applymap(lambda x: isinstance(x, str))["width"], "width"
    ] = np.nan
