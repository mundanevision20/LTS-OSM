...
G2.remove_edges_from(all_lts[(all_lts["lts"] > 2) & (all_lts["lts"] == 0)].index)
G3.remove_edges_from(all_lts[(all_lts["lts"] > 3) & (all_lts["lts"] == 0)].index)
...
center_node = ox.distance.nearest_nodes(
    G1, x, y
)  # use the same starting node for each graph
...
ax.scatter(
    [point_geom_proj.x], [point_geom_proj.y], marker="*", s=50, color="k", zorder=2
)
...
cmap = matplotlib.colors.ListedColormap(iso_colors[::-1]).with_extremes(
    over="0.25", under="0.75"
)
...
plt.savefig(
    "%s_isochrone_times_lts_remove_nodes_%s_time_%s.pdf"
    % (city, remove_nodes, trip_time)
)
plt.savefig(
    "%s_isochrone_times_lts_remove_nodes_%s_time_%s.png"
    % (city, remove_nodes, trip_time),
    dpi=300,
)
# -
