...
from lts_functions import (
    biking_permitted,
    is_separated_path,
    is_bike_lane,
    parking_present,
    bike_lane_analysis_no_parking,
    bike_lane_analysis_with_parking,
    mixed_traffic,
)
...
# load the data
f = open("nuremberg.osm")  # a larger bounding box for the city
data = json.load(f)
...
tag_value_counts = (
    tags_df.value_counts().reset_index()
...
all_lts = pd.concat(
    [separated_edges, parking_lts, no_parking_lts, lts_no_lane, gdf_not_allowed]
)
